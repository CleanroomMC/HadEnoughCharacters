            ...
            if (entry.getName().endsWith(".class")) {
                try (InputStream is = f.getInputStream(entry)) {
                    ...
                    if (size > Integer.MAX_VALUE) {
                        HechCore.LOG.info("Class file " + entry.getName()
                    ...
                    HechCore.LOG.info("Fail to read file " + entry.getName()
                ...
                try (InputStream is = f.getInputStream(entry)) {
                    ...
                    HechCore.LOG.info("Fail to read mod info in jar file " + f.getName() + ", skip.");
                }
            }
        });
    ...
    private static void scanClass(InputStream is, Consumer<String> string, Consumer<String> equals, Consumer<String> regexp,
        ...
        try {
            ...
            if (classNode.name != null) {
                HechCore.LOG.info("File decoding of class " + classNode.name + " failed. Try to continue.");
            } else {
